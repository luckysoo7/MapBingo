...
test('첫 방문 시 (캐시 없음) 업로드 오버레이가 표시된다', async ({ page }) => {
  ...
  await page.evaluate(() => {
    ...
    indexedDB.deleteDatabase('mapbingo')
  })
...
test('IndexedDB에 데이터 저장 후 리로드하면 오버레이 없이 지도 복원된다', async ({ page }) => {
  ...
  await page.evaluate(() => { localStorage.setItem('mapbingo-cache', 'on') })

  // mock 데이터를 스토어에 넣고 IndexedDB에 저장
  await page.evaluate(({ photos, stats }) => {
    const store = window.__mapbingoStore?.getState()
    ...
    return new Promise<void>((resolve, reject) => {
      const req = indexedDB.open('mapbingo', 1)
